...
if (ProcessExist("ABPS.exe")) {
	...
	SendText "N:\99_Temporäres_Zeug\gfu_Abarbeitungsstand.xlsx"
	Send "{Enter}"
...
; AUF EXCEL WARTEN
	...
	SetTitleMatchMode "RegEx"  	; REGEX
        Sleep 300  ; Kurz warten
	Send "!{F4}"		; ALt F4
	;Winclose("gfu_Abarbeitungsstand(\.xlsx)? - Excel")
...
; WEITER IN ABPS
	...
	MenuSelect "ABPS  -  Nutzer: ",, "Auftragsbearbeitung", "2&"  , "1&"       ; Fertigungsauftrag"
	...
	ControlFocus "TDBGrid1"
        ...
        Send "!{Backspace}"		; ALt Backspace
        WindowWait("Speichern unter",1)
	SendText "N:\99_Temporäres_Zeug\gfu_Fertigungsauftrag.xlsx"
	Send "{Enter}"
	...
	WindowWait("gfu_Fertigungsauftrag(\.xlsx)? - Excel", "RegEx")
        Sleep 300  ; Kurz warten
	;SetTitleMatchMode "RegEx"  	; REGEX
	;Winclose("gfu_Fertigungsauftrag(\.xlsx)? - Excel")
	Send "!{F4}"		; ALt F4
